...
import os
import sys
# Let the app import the local prediction module without packaging the repo first.
...
st.caption("Dynamic Complaint Intelligence System")
...
if st.button("Analyze Complaint", use_container_width=True):
    if complaint.strip():
        ...
        # Surface the main classifier outputs before the richer narrative details.
        ...
        for i, case in enumerate(result["similar_cases"], start=1):
            # Keep long complaint narratives readable inside collapsible sections.
        ...
        with st.expander("Top-3 Model Predictions"):
            st.write("**Products**")
            for name, p in result["top3_products"]:
                st.write(f"- {name}: {p*100:.1f}%")

            st.write("**Issues**")
            for name, p in result["top3_issues"]:
                st.write(f"- {name}: {p*100:.1f}%")
